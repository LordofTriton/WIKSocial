    ...
    return (
        ...
        >
            <div className="w-full bg-white dark:bg-eerie-black">
                ...
                <Editor onChange={(data) => setPostContent(data)} />

                <div className="w-full flex flex-row justify-start items-center pl-8">
                    <button className="text-lg font-medium text-white rounded-xl bg-celestial-blue px-4 py-2" onClick={() => createPost()}>Publish</button>
...

                        {
                            showOptionsDrop && (
                                <div ref={optionsDropRef} className="w-48 bg-white dark:bg-eerie-black rounded-xl p-2 absolute bottom-full left-0 shadow-md border border-transparent dark:border-glass z-10">
                                    ...
                                    <div className="flex flex-row justify-between items-center px-2 hover:bg-whitesmoke py-2 rounded-xl cursor-pointer text-night dark:text-gray-300 dark:hover:bg-ash" onClick={(e) => { e.stopPropagation(); setAdultContent(!adultContent); setShowOptionsDrop(false); }}>
                                        <TbRating18Plus className="w-5 h-5 ml-1" />
                                        <span className="text-sm flex-1 font-medium px-3 py-0">{adultContent ? "Unmark 18+" : "Mark 18+"}</span>
                                    </div>
                                    <div className="flex flex-row justify-between items-center px-2 hover:bg-whitesmoke py-2 rounded-xl cursor-pointer text-night dark:text-gray-300 dark:hover:bg-ash" onClick={(e) => { e.stopPropagation(); }}>
