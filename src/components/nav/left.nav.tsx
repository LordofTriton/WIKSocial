...
const menuButtons = [
    {
        label: "Popular",
        icon: <OutlineFireIcon className="w-6 h-6 text-night" />,
    ...
    {
        label: "Fresh",
        icon: <OutlineClockIcon className="w-6 h-6 text-night" />,
    ...
    {
        label: "My Feed",
        icon: <PiCheckSquareOffset className="w-6 h-6 text-night" />,
    ...
    {
        label: "Messages",
        icon: <OutlineChatBubbleLeftEllipsisIcon className="w-6 h-6 text-night" />,
    ...
    {
        label: "Rating",
        icon: <PiCompass className="w-6 h-6 text-night" />,
...
export const LeftNav = () => {
    ...
    return (
        <div className="hidden md:block md:w-2/12 xl:w-2/12 sticky top-0 h-screen pt-5" onClick={() => toggleDarkMode()}>
            <div>
                {
                    menuButtons.map((button, index) =>
                        <div className={`flex flex-row ${pathname.includes(button.route) ? "bg-white" : "hover:bg-white"} rounded-xl justify-between items-center px-3 py-3 mb-1 cursor-pointer`} key={index}>
                            ...
                            <span className="text-base text-night flex-1 font-medium px-3 py-0">{button.label}</span>
